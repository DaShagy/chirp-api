package com.juanjoseabuin.chirp.infra.rate_limiting

import com.juanjoseabuin.chirp.infra.config.NginxConfig
        ...
        return try {
            val inetAddr = when {
                ...
                trimmedIp.matches(Regex("\\d+\\.\\d+\\.\\d+\\.\\d+")) -> Inet4Address.getByName(trimmedIp)
                    ...
                    logger.warn("Invalid IP format in $headerName: $trimmedIp from $proxyIp")
                    return null
                }
            }
            ...
            logger.warn("Invalid IP format in $headerName: $trimmedIp from $proxyIp", e)
            null
        }
    }
